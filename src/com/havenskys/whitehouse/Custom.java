...
public class Custom {
	...
	// CUSTOM
	public static String APP = "WhiteHouse";
	public static String WHO = "WhiteHouse";
	public static boolean PUBLISH = true;
	public static boolean STUDYFEED = false;
	public static boolean FREEVERSION = false;
	public static String EMAIL = "info@whitehouse.gov";
	public static int LITTLEICON = R.drawable.whitehouse;
	public static int TOPICON = R.drawable.usseal;
	public static String MAINURI = "com.havenskys.whitehouse";
	...
	public void loadlist(listView view) {
		...
		String[] columns = new String[] {"_id", "title", "datetime(date,'localtime') as date", "author"};
        String[] from = new String[]{"title", "date", "author" };
        int[] to = new int[]{R.id.listrow_title, R.id.listrow_date, R.id.listrow_author };
	...
	public void parseEntries(String who, String httpPage) {
		...
		int syncLoad = mSharedPreferences.contains("syncload") ? mSharedPreferences.getInt("syncload",6) : 6;
		
		// CUSTOM
		String entryTag = "item";
		String[] parseColumns = new String[]{ Custom.TITLE, Custom.LINK, Custom.DATE, Custom.AUTHOR, Custom.CONTENT };
		String[] parseTags = new String("title,link,pubDate,dc:creator,description").split(",");
		...
		for( int i = 0; i < input.length; i++){
			...
			if(STUDYFEED || i < 50){w(TAG,"Line("+i+") " + input[i] );}
			if( input[i].contains("<"+entryTag+">") ){
				...
				for(i++; i < input.length; i++){
					if( input[i].length() == 0 ){ continue; }
					if(STUDYFEED || i < 50){w(TAG,"Item Line("+i+") " + input[i] );}
				...
				for(parseI = 0; parseI < parseLen; parseI++){
					...
					if( s != null ){/*
						if(parseColumns[parseI].contains(Custom.AUTHOR) || parseColumns[parseI].contains(Custom.TITLE)){
							if( s.contains("&amp;") ){
								s = s.replaceAll("&amp;","&");
								parsedValues.put(parseColumns[parseI], s);
							}
							if( s.contains("&#039;") ){
								s = s.replaceAll("&#039;", "'");
								parsedValues.put(parseColumns[parseI], s);
	...
	public void setEntryNotification(String who, long rowid, int icon, String title, String details, String topscroll){
		...
		Intent intentJump2 = new Intent(mContext, com.havenskys.whitehouse.Start.class);
	...
	public void setServiceNotification(String who, int icon, String title, String details, String topscroll){
		...
		Intent intentJump = new Intent(mContext, com.havenskys.whitehouse.Stop.class);
